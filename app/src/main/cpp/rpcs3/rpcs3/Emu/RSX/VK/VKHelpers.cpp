...
namespace vk
{
	...
	void set_current_renderer(const vk::render_device &device)
	{
...

		{
			...
			{
				info.usage = usage;
				CHECK_RESULT(_vkCreateBuffer(*g_render_device, &info, nullptr, &tmp));

				_vkGetBufferMemoryRequirements(*g_render_device, tmp, &memory_reqs);
				...
				_vkDestroyBuffer(*g_render_device, tmp, nullptr);
			}
		}
