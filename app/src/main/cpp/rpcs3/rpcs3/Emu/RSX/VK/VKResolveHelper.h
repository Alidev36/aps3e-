...
namespace vk
{
	...
	struct depth_resolve_base : public overlay_pass
	{
		...
		{
			_vkCmdPushConstants(cmd, program->layout(), VK_SHADER_STAGE_FRAGMENT_BIT, 0, static_parameters_width * 4, static_parameters);
		}
	...
	struct stencilonly_resolve : depth_resolve_base
	{
		...
		{
			_vkCmdClearAttachments(cmd, 1, &clear_info, 1, &region);

			for (s32 write_mask = 0x1; write_mask <= 0x80; write_mask <<= 1)
			{
				_vkCmdSetStencilWriteMask(cmd, VK_STENCIL_FRONT_AND_BACK, write_mask);
				_vkCmdPushConstants(cmd, program->layout(), VK_SHADER_STAGE_FRAGMENT_BIT, 8, 4, &write_mask);
	...
	struct stencilonly_unresolve : depth_resolve_base
	{
		...
		void emit_geometry(vk::command_buffer& cmd, glsl::program* program) override
		{
			_vkCmdClearAttachments(cmd, 1, &clear_info, 1, &clear_region);
