...
namespace vk
{
	namespace FidelityFX
	{
		...
		void easu_pass::configure(const vk::command_buffer& cmd)
		{
			...
			_vkCmdPushConstants(cmd, m_program->layout(), VK_SHADER_STAGE_COMPUTE_BIT, 0, push_constants_size, m_constants_buf);
		}
		...
		rsx::flags32_t mode)
	{
		...
		if (mode & UPSCALE_AND_COMMIT)
		{
			...
			_vkCmdBlitImage(cmd, src_image->value, src_image->current_layout, target_image, target_image_layout, 1, &output_request, VK_FILTER_LINEAR);
