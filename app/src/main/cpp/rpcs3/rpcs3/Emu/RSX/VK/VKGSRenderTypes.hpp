...
namespace vk
{
	...
	struct command_buffer_chunk : public vk::command_buffer
	{
		...
		bool poke()
		{
			...
			if (_vkGetFenceStatus(pool->get_owner(), m_submit_fence->handle) == VK_SUCCESS)
			{
				lock.upgrade();
	...
	struct frame_context_t
	{
		...
		void init(VkDevice dev)
		{
			...
			_vkCreateSemaphore(dev, &semaphore_info, nullptr, &present_wait_semaphore);
			_vkCreateSemaphore(dev, &semaphore_info, nullptr, &acquire_signal_semaphore);
		}

		void destroy(VkDevice dev)
		{
			_vkDestroySemaphore(dev, present_wait_semaphore, nullptr);
			_vkDestroySemaphore(dev, acquire_signal_semaphore, nullptr);
		}
